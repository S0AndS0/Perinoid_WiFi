...
if [ -e "${Var_main_script_name}" ]; then
	echo "# ${Var_script_name} running: sudo ./${Var_main_script_name} --debug-level='9' --ovpns-config-yn='yes' --ovpns-listen-ip=\"${Var_local_ip}\" --ovpns-listen-port=\"${Var_listen_port}\""
	sudo ./${Var_main_script_name} --debug-level='9' --ovpns-config-yn='yes' --ovpns-auto-start-yn='yes' --ovpns-listen-ip="${Var_local_ip}" --ovpns-listen-port="${Var_listen_port}"
	_exit_status=$?
