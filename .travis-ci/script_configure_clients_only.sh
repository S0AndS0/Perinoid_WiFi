...
if [ -e "${Var_main_script_name}" ]; then
	echo "# ${Var_script_name} running: ./${Var_main_script_name} --debug-level='9' --ovpnc-config-yn='yes' --ovpnc-pass-yn='yes' --ovpnc-host-ports=\"${Var_external_ip}:${Var_listen_port}\" --ovpnc-names=\"${Var_client_users_list}\""
	./${Var_main_script_name} --debug-level='9' --ovpnc-config-yn='yes' --ovpnc-pass-yn='yes' --ovpnc-host-ports="${Var_external_ip}:${Var_listen_port}" --ovpnc-names="${Var_client_users_list}"
	_exit_status=$?
